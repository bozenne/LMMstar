...
##'
##' ## select null hypotheses & combine
##' AAA <- anova(e.lmm1, ci = TRUE, effect = c("X1|X2,X3"="X1=0","X2|X1,X3"="X2=0"))
##' BBB <- anova(e.lmm2, ci = TRUE, effect = c("X1|X8,X9"="X1=0"))
##' ZZZ <- rbind(AAA,BBB)
##' summary(ZZZ) ## by default robust standard errors are used
##' 
##' ## select null hypotheses & combine
##' AA <- anova(e.lmm1, ci = TRUE, effect = c("X1|X2,X3"="X1=0","X2|X1,X3"="X2=0"),
##'              robust = FALSE)
##' BB <- anova(e.lmm2, ci = TRUE, effect = c("X1|X8,X9"="X1=0"), robust = FALSE)
##' ZZ <- rbind(AA,BB)
##' summary(ZZ) ## model-based like standard errors are used
