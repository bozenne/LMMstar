...
##' @param type.residual [character] Should the raw residuals be output (\code{"response"}), the Pearson residuals (\code{"pearson"}), the studentized residuals (\code{"studentized"}), or normalized residuals (\code{"normalized"}).
...
residuals.lmm <- function(object, type.residual = "response", format = "long",
    ...
    if(identical("all",tolower(type.residual))){
        type.residual <- c("response","studentized","pearson","normalized","normalized2")
    }
    type.residual <- match.arg(type.residual, c("response","studentized","pearson","normalized","normalized2"), several.ok = (format=="long"))
    ...
    if(type.object == "lmm"){
        ...
        if(!is.null(p)){
            ...
            if(any(type.residual %in% c("studentized","pearson","normalized","normalized2"))){
        ...
        if("studentized" %in% type.residual || "normalized2" %in% type.residual){
            ...
            if(!is.null(design$precompute.XX)){
                ...
                for (iPattern in 1:n.pattern) { ## iPattern <- 1
                    ...
                    iX <- matrix(unlist(design$precompute.XX$pattern[[iPattern]]), nrow = iTime2, ncol = dim(design$precompute.XX$pattern[[iPattern]])[3], byrow = FALSE)
                    tX.precision.X <- tX.precision.X + (as.double(iOmega) %*% iX)[as.double(design$precompute.XX$key)]
                }
            }else{
        ...
        if(is.null(data) && length(object$index.na)>0){
            ...
            level.cluster <- factor(design$cluster$levels[index.cluster], levels = design$cluster$levels)
