...
[[#SM:likelihood:log]]. Key assumptions are:
- \(\beta\) and \(\gamma\) are distinct sets of parameters and the
...
- time elapsed between follow-up times are independent of current and
...
- by default 'model-based' standard errors are evaluated, as the
  ...
  [[#SM:likelihood:hessian]] for an explicit formula. Using the observed
  ...
  results of software.
- 'robust' standard error are evaluated as the square root of the
  diagonal elements of \newline \({\Information(\Theta)^{-1} \Big(\sum_{i=1}^n
  \Score_i(\Theta)\trans{\Score}_i(\Theta)\Big)\Information(\Theta)^{-1}}
  \). \Warning Inflated type 1 error rate is expect in small samples
...
- adjustment for multiple comparisons can be performed using various
...
** Hessian
:PROPERTIES:
:CUSTOM_ID: SM:likelihood:hessian
:END:
