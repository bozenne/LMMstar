...
so does not affect the relation between the outcome (here =LifeExp=)
and the exposure of interest (here =Income=). One can then get a
...
ggarrange(gg.obs + coord_cartesian(ylim=c(68,74)) + theme(plot.title = element_text(size = 10)),
          gg.pres + coord_cartesian(ylim=c(68,74)) + theme(plot.title = element_text(size = 10)),
          autoplot(e.lm, time = "income", type = "partial", var = c("(Intercept)","income"))$plot + coord_cartesian(ylim=c(68,74)) + ggtitle("(C):  had states no illiteracy, no murder, low education - same as (B)") + theme(plot.title = element_text(size = 10)),
          autoplot(e.lm, time = "income", type = type, var = c("(Intercept)","income"))$plot + coord_cartesian(ylim=c(68,74)) + ggtitle("(D): had states average illiteracy, murder rate, and education") + theme(plot.title = element_text(size = 10)),
          nrow = 2, ncol = 2)
dev.off()
#+END_SRC
...
actually displaying the graph one can use =autoplot= to only output the result:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
...
ggarrange(autoplot(e.lm, time = "income", type = "partial", var = c("(Intercept)","income"), ci.alpha = 0.25)$plot + theme(plot.title = element_text(size = 10)),
          autoplot(e.lm, time = "murder", type = "partial", var = c("(Intercept)","murder"), ci.alpha = 0.25)$plot + theme(plot.title = element_text(size = 10)))
dev.off()
#+END_SRC
...
ggarrange(autoplot(e.lm, time = "income", type = "partial", var = "income", ci.alpha = 0.25)$plot + theme(plot.title = element_text(size = 10)),
          autoplot(e.lm, time = "murder", type = "partial", var = "murder", ci.alpha = 0.25)$plot + theme(plot.title = element_text(size = 10)))
dev.off()
#+END_SRC
...
#+BEGIN_SRC R :exports none :results output :session *R* :cache no
gg <- ggarrange(autoplot(e.lmI, time = "income", type = "partial", var = c("(Intercept)","income","edu"))$plot + ggtitle("Reference: illiteracy=0, murder=0 (interaction model)") + theme(plot.title = element_text(size = 10)),
                autoplot(e.lm, time = "income", type = "partial", var = c("(Intercept)","income","edu"))$plot + ggtitle("Reference: illiteracy=0, murder=0 (no interaction model)") + theme(plot.title = element_text(size = 10)),
...
to define a new reference (i.e. drop =edu=):
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
typeI <- "partial"
...
and make sure that the covariates (here =lesion=) has no missing values:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
summarize(visual+treat.f+week+lesion~1, data = armd.long, na.rm = TRUE)
#+END_SRC

#+RESULTS:
:          outcome observed missing       mean         sd min q1 median q3 max
: 1         visual     1107      93 49.8590786 17.5374055   3 37     51 63  85
: 2         lesion     1195       5  1.8912134  0.9314250   1  1      2  3   4
: 3 treat.f:Active     1200       0  0.5041667  0.5001911   0  0      1  1   1
: 4         week:4     1200       0  0.2000000  0.4001668   0  0      0  0   1
: 5        week:12     1200       0  0.2000000  0.4001668   0  0      0  0   1
: 6        week:24     1200       0  0.2000000  0.4001668   0  0      0  0   1
: 7        week:52     1200       0  0.2000000  0.4001668   0  0      0  0   1

This is not the case here so we will subset the dataset to exclude missing values in the covariates:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
armd.longCC <- armd.long[!is.na(armd.long$lesion),]
#+END_SRC

#+RESULTS:

Then we can fit a linear mixed model:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
e.lmm <- lmm(visual ~ week + week:treat.f + lesion, data = armd.longCC,
