...
- the function =lmm= is the main function of the package which fits
  linear mixed models. The user can interact with /lmm/ objects using:
    ...
    + =plot= to obtain a diagnostic plots or a graphical display of the fitted values.
...
The marginal mean at each timepoint can be obtainet using =dummy.coef=:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
dummy.coef(eUN.lmm)
#+END_SRC
...
#+begin_src R :file ./figures/diag-correlation.pdf :results graphics file :session *R* :cache no
...
the average curve (over glucagon values). With the =autoplot= method,
