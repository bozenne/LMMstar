...
* Equivalence between LMM and LM
...
complete case approach [fn::in the former the likelihood is evaluated
...
#+BEGIN_SRC R :exports none :results output :session *R* :cache no
yyy <- lmmCC(e.change32, repetition = changeG32 ~ glucagonAUC3-glucagonAUC2)
model.tables(yyy)
#+END_SRC
...
** Regressing the change against another change 
...
1   1      weight1  127.20
21  1      weight4  108.10
41  1 glucagonAUC1 5032.50
61  1 glucagonAUC4 9249.45
2   2      weight1  165.20
22  2      weight4  132.00
#+end_example
...
estimate(e.lmm4, function(p){
  ...
  k <- sqrt(Sigma.change[2,2]/Sigma.change[1,1])
  c(cor = cov2cor(Sigma.change)[1,2],
    beta = k*cov2cor(Sigma.change)[1,2])
})
...
#+BEGIN_SRC R :exports none :results output :session *R* :cache no
yyy <- lmmCC(e2.change41, repetition = list(changeG41 ~ glucagonAUC4-glucagonAUC1,
                                            changeW41 ~ weight4-weight1))
yyy$lmm.change
#+END_SRC

#+RESULTS:
:   estimate       se       df     lower    upper   p.value
: 1 380.3556 198.3453 2.798661 -277.3518 1038.063 0.1575655
...
#+BEGIN_SRC R :exports none :results output :session *R* :cache no
xxx <- lmmCC(e.change32, repetition = changeG32 ~ glucagonAUC3-glucagonAUC2)
coef(xxx)
#+END_SRC
...
:  - 2 observations with missing data (2 clusters) 
:  - 0 missing repetitions (0 clusters)
:  (Intercept)         time      weight1 time:weight1 
:  -36283.0356   38101.9400     257.7767    -217.2672
...
#+BEGIN_SRC R :exports none :results output :session *R* :cache no
yyy <- lmmCC(e2.change32, repetition = list(changeG32 ~ glucagonAUC3-glucagonAUC2,
                                            changeW32 ~ weight3-weight2))
yyy$lmm.change
#+END_SRC
...
:  - 0 missing repetitions (0 clusters)
:    estimate       se       df     lower    upper   p.value
: 1 -319.9597 1117.962 2.161834 -4801.046 4161.126 0.7998465
...
#+BEGIN_SRC R :exports none :results output :session *R* :cache no
yyy <- lmmCC(e2g.change32, repetition = list(changeG32 ~ glucagonAUC3-glucagonAUC2,
                                             changeW32 ~ weight3-weight2))
yyy$lmm.change
#+END_SRC
...
:  - 0 missing repetitions (0 clusters)
:    estimate       se       df     lower    upper   p.value
: 1 -783.8895 1081.557 2.338122 -4848.483 3280.703 0.5342415
...
using =lmm= with a small hack: using a block compound symmetry
