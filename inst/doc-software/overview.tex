...
\label{sec:orgb1fef4a}
\subsection{Classical covariance patterns}
\label{sec:orga4ef5b9}
...
\noindent Fit a linear mixed model with \textbf{toeplitz} covariance matrix:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
eTOE.lmm <- lmm(weight ~ time*group,
		repetition = ~time|id, structure = "TOEPLITZ",
		data = gastricbypassL)
eTOE.lmm
...
Fixed effects: weight ~ time + glucagon 
                ...
                estimate    se   df   lower   upper p.value    
    (Intercept)   132.98 4.664 19.8 123.243 142.717 < 0.001 ***
    timeB1w       -7.882 0.713 19.2  -9.374   -6.39 < 0.001 ***
    timeA1w      -11.788 1.018 21.6   -13.9  -9.676 < 0.001 ***
    timeA3m      -26.122 1.656 18.8 -29.591 -22.654 < 0.001 ***
...
variance-covariance matrix can then be decomposed as \(\Omega = Z
\Omega_1 \trans{Z} + \Omega_2\) where \(Z\) is the design matrix
associated to the possibly nested clustering factors, \(\Omega_1\) is
the variance-covariance of the random effects and \(\Omega_2\) the
residual-variance covariance conditional to the random
effects. Denoting by \(\varepsilon_i\) the vector of marginal
residuals relative to individual \(i\) with variance-covariance matrix
\(\Omega_i\), \(\eta_{ij} = \omega_{2j} Z_{ij}
\Omega_i^{-1}\varepsilon_i\) is its \(j\)-th random effect with design
matrix \(Z_{ij}\) and variance \(\omega_{2j}\). This is what the
\texttt{coef} method returns when setting the argument \texttt{effects} to
\texttt{"ranef"}:
