...
\label{sec:orge89d2c8}

\subsection{Summary statistics}
\label{sec:org1326b4b}
...
\label{sec:org31f854c}
\subsection{Classical covariance patterns}
\label{sec:org2bfeaba}
...
e.ANCOVA <- lm(weight.A3m ~ weight.B3m + group, data = gastricbypassW)
...
weight.B3m   0.8236279 0.06411563 12.8459772 3.524665e-10
...
\section{Equivalence between LMM and LM}
\label{sec:org6a70ad1}

\subsection{Regressing the change}
\label{sec:org75ebf00}
...
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
index.missing32 <- which(is.na(gastricbypassW$changeG32))
index.missing32
\end{lstlisting}
...
LMM uses a full information approach while a linear model perform a
complete case approach \footnote{in the former the likelihood is evaluated
using all observations, even those from individuals with some (but not
all) missing outcome values: baseline is used even if follow-up is
missing. In the later the likelihood is only evaluated on individuals
with no missing outcome values: if follow-up is missing then baseline
is not used}:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
which is the same as removing the two individuals with missing data:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
coef(lm(changeG32 ~ weight1, data = gastricbypassW[-index.missing32,]))
\end{lstlisting}
...
The LMM would give (here slightly) different estimates:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
These estimates can be retrived using a linear model where the mixed
model has imputed the conditional expectation of the missing values
given the observed value and the estimated model parameters:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\subsection{Regressing the change against another change}
\label{sec:org080f1c7}
...
This problem can be recast as mixed model with all measurement as
outcomes, and where the parameter of interest is 
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
keep.col <- c("id","weight1","weight4","glucagonAUC1","glucagonAUC4")
gastricbypassL4 <- melt(gastricbypassW[,keep.col], id.vars = "id")

e.lmm4 <- lmm(value ~ variable,
              repetition = ~variable|id, structure = "UN",
...
\label{sec:org59d4c9d}
\subsection{Complete case approach}
\label{sec:orga9088e7}
...
\label{sec:org71602ae}

\subsection{nlme package}
\label{sec:org702b7f1}
...
\begin{itemize}
\item it uses the expected information matrix to quantify uncertainty
instead of the observed information matrix.
\item it uses the Kenward and Roger method for computing the degrees of
freedom instead of the Satterthwaite approximation:
\item it implements different covariance patterns
\item it is faster but provides less support to work with the output of
the mixed model (e.g. no \texttt{residual} method)
\end{itemize}

\subsection{effectsize package (\(R^2\) or \(\eta^2\))}
\label{sec:org178b72c}
